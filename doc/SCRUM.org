...
- *Testable*: Every feature has an automated test (139 tests, TAP format)
...
emacs_from_scratch/
├── src/
│   └── init.el                          # Main config (787 lines, 24 KB)
├── test/
│   ├── test_init.el                     # Test suite (650 lines, 20 KB, 139 tests)
...
├── install_emacs_config_dependencies.sh # System deps installer (40 lines, 750 B)
...
| Key         | Command               | Description         |
|-------------+-----------------------+---------------------|
| =C-<TAB>=   | =tempo-complete-tag=  | Expand snippet      |
...
- *Runner*: =./test/run_tests.sh= or =emacs --batch -l test/test_init.el=
...
| 16. Media & Documents |   5 | pdf-tools, image-dired+, mermaid-mode, svg-tag-mode  |
