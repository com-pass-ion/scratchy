...
emacs_from_scratch/
├── src/
│   └── init.el                          # Main config (720 lines, 24 KB)
├── test/
│   ├── test_init.el                     # Test suite (570 lines, 20 KB, 123 tests)
...
16 numbered sections in =src/init.el=:
...
| 15  | Language Modes | markdown, yaml, json, toml, nix, dockerfile | Config file support |
